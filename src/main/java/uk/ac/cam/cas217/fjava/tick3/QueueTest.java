...
public class QueueTest {
    ...
    public static void main(String[] args) {
        ...
        new QueueTest(new UnsafeMessageQueue<>(), 1, 1).run();
        new QueueTest(new UnsafeMessageQueue<>(), 3, 1).run();
        new QueueTest(new UnsafeMessageQueue<>(), 1, 3).run();
        new QueueTest(new UnsafeMessageQueue<>(), 3, 3).run();

        System.out.println("** SAFE ** ");
        new QueueTest(new SafeMessageQueue<>(), 1, 1).run();
        new QueueTest(new SafeMessageQueue<>(), 3, 1).run();
        new QueueTest(new SafeMessageQueue<>(), 1, 3).run();
        new QueueTest(new SafeMessageQueue<>(), 3, 3).run();
    }
}
