...
//        return object.getColour();
    ...
    private Vector3 illuminate(Scene scene, Sphere object, Vector3 P, Vector3 N) {
        ...
        Vector3 V = new Vector3(0).subtract(P).normalised();
        Vector3 R = L.reflectIn(N);

        Vector3 ambient = C_diff.scale(I_a);
        Vector3 diffuse =  C_diff.scale(I).scale(k_d * Math.max(0, N.dot(L)));
        Vector3 specular = C_spec.scale(I).scale(k_s * Math.pow(Math.max(0, R.dot(V)), n));
